...
R
```
The total gains $G_{n}$ can be defined as the difference between the sum of total stakes $s_{n}$ and the $n$ negative outcomes associated with $s_{i}$ stakes, i.e. it is the sum of negative bets $-p_{i}S_{n}$, such that $G_{n} = s_{n} - \sum_{1}^{n}p_{i}s_{i}$.
...
Regardless, and to return to the point of this section, it is crucial to note that if the series of event probabilities are incoherent, and thus do indeed lead to an invertible matrix, we can solve for an unknown set of stakes $s_{n}$, suggesting that we can strucure our bets in such a way as to always obtain a net gain $G_{n}$, regardless of the outcome. First, we provide the following arbitrary definite gains $G_{n}$ for each stake $s_{n}$,
```{r, echo = TRUE}
G <- c(3, 1, 4)
G
```
